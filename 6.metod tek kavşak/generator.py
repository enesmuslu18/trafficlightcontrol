...
class TrafficGenerator:
    ...
    def generate_routefile(self, seed):
        ...
        with open("intersection/sehrekustu.rou.xml", "w") as routes:
            print("""<routes>
            <vType accel="1.0" decel="4.5" vClass= "passenger" id="standard_car" length="5.0" minGap="2.5" maxSpeed="25" sigma="0.5" color= "cyan" />
            ...
            for car_counter, step in enumerate(car_gen_steps):
                ...
                if personal_or_public < 0.75:  # choose car type: personal or public - 75% of times personal car
                    ...
                    if personal_car == 1:
                        print('    <vehicle id="car_D_B_%i" type="standard_car" route="D_B" depart="%s" departLane="random" departSpeed="8" />' % (car_counter, step), file=routes)
                    elif personal_car == 2:
                        print('    <vehicle id="car_D_K_%i" type="standard_car" route="D_K" depart="%s" departLane="random" departSpeed="8" />' % (car_counter, step), file=routes)
                    elif personal_car == 3:
                        print('    <vehicle id="car_K_G_%i" type="standard_car" route="K_G" depart="%s" departLane="random" departSpeed="8" />' % (car_counter, step), file=routes)
                    elif personal_car == 4:
                        print('    <vehicle id="car_K_B_%i" type="standard_car" route="K_B" depart="%s" departLane="random" departSpeed="8" />' % (car_counter, step), file=routes)
                    elif personal_car == 5:
                        print('    <vehicle id="car_B_D_%i" type="standard_car" route="B_D" depart="%s" departLane="random" departSpeed="8" />' % (car_counter, step), file=routes)
                    elif personal_car == 6:
                        print('    <vehicle id="car_B_K_%i" type="standard_car" route="B_K" depart="%s" departLane="random" departSpeed="8" />' % (car_counter, step), file=routes)
